...
public class DataMediaItem extends BaseObservable {
    ...
    public final ObservableInt imgid = new ObservableInt(-1);
            ...
            if (duration.get() > 0) {
                ...
                time.set(
                        String.format(Locale.getDefault(), "%d %s",
                                d,
                                (b ?
                                   AppMain.getAppResources().getQuantityString(R.plurals.plurals_minutes, d) :
                                   AppMain.getAppResources().getQuantityString(R.plurals.plurals_second, d)
                                )
                        )
                );
