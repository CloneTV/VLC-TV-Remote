...
function parse_filename(meta)
    ...
    vlc.item:set_meta("name", showName)
...
function parse_nfo(s)
	...
	vlc.item:set_meta("nfo", vlc.strings.make_uri(s))
	...
	vlc.item:set_meta("premiered", txt:match("<premiered>([^<]+)"))
	vlc.item:set_meta("rating", txt:match("<rating>([^<]+)"))
	vlc.item:set_meta("votes", txt:match("<votes>([^<]+)"))
