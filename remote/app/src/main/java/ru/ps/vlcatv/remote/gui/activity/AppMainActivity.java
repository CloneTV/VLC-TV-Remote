...
public class AppMainActivity extends FragmentActivity implements SettingsInterface {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            AppMain.getStatus().setCallbackChanged(this);
    ...
    private void event_BTN_TITLE() {
        ...
        AppMain.getStatus().AppInfo.set(
                !AppMain.getStatus().AppInfo.get()
        );
    }
    ...
    private void event_BTN_SETUP() {
        ...
        AppMain.getStatus().AppSetup.set(
                !AppMain.getStatus().AppSetup.get()
        );
    }
    ...
    public void OnClickBtn(int id, String s) {
        ...
        switch (id)
        {
            ...
            case DataSharedControl.BTN_MENU: {
                AppMain.printError("Runnable r that are in the message queue. If you don't want to keep a reference of the runnable, you could simply call");
                break;
            }
            default: {
                ...
                if (!Utils.isempty(cmd)) {
                    ...
                    AppMain.getStatus().clickStateChange(id);
                }
                break;
            }
        }
        ...
        public void run() {
            try {
                if (!AppMain.getSettings().isempty()) {
                    AppMain.getRequestStatus();
                    AppMain.getStatus().clickStateChange(DataSharedControl.GET_STAT);
                }
