    ...
    private static class CallbackMediaItem implements Callback<JSONObject> {
        ...
        public void onResponse(Call<JSONObject> call, Response<JSONObject> response) {
            try {
                ...
                Log.e("CallbackMediaItem", "desc=" + AppMain.getStatus().MmItem.description.get());
                Log.e("CallbackMediaItem", "date=" + AppMain.getStatus().MmItem.date.get());
                Log.e("CallbackMediaItem", "poster=" + AppMain.getStatus().MmItem.poster.get());
                Log.e("CallbackMediaItem", "descVisible=" + AppMain.getStatus().MmItem.descVisible.get());
                Log.e("CallbackMediaItem", "imgVisible=" + AppMain.getStatus().MmItem.imgVisible.get());
    ...
    private static class CallbackMediaItems implements Callback<JSONObject> {
        ...
        public void onResponse(Call<JSONObject> call, Response<JSONObject> response) {
            try {
                ...
                for (int i = 0; ((i < array.length()) && (i < AppMain.getStatus().MmItems.length)); i++) {
                    JSONObject o = array.optJSONObject(i);
                    if (o != null)
                        AppMain.getStatus().MmItems[i] = new DataMediaItem(o);
                }
